...
# Every event name this plugin has ever registered in hooks.json. None have been
# retired yet; when one is, it stays in this tuple and moves to RETIRED.
HISTORICAL_EVENTS = ("SessionStart", "Stop", "SessionEnd")
...
class HookExitCodeTest(unittest.TestCase):
    ...
    def test_launcher_survives_a_broken_interpreter(self):
        ...
        proc = self.run_hook("--event", "Stop", "--data-dir", self.data_dir,
                             env={"BTT_PYTHON": "/nonexistent/python3"})
        self.assert_never_blocks(proc, "BTT_PYTHON=/nonexistent/python3")
...
class HookConfigTest(unittest.TestCase):
    ...
    def test_every_registered_event_has_a_handler(self):
        ...
        for event in self.hooks:
            self.assertIn(event, ingest.EVENTS,
    ...
    def test_retired_names_are_never_reused_as_handlers(self):
        self.assertEqual(set(ingest.RETIRED) & set(ingest.EVENTS), set())

    def test_every_historical_event_is_known_or_retired(self):
        """Guards the rule: retire an event, never delete it."""
        accounted = set(ingest.EVENTS) | set(ingest.RETIRED)
        ...
        for event, groups in self.hooks.items():
            for group in groups:
                for hook in group["hooks"]:
                    self.assertIn('/bin/btt" ingest ', hook["command"],
                                  f"{event} invokes something other than "
                                  f"`btt ingest`: {hook['command']}")
