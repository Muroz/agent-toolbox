"""Transcript parsing for branch-token-tracker.
...
transcripts rather than assumed:
  ...
  * Subagent usage is NOT in the main transcript as sidechain records. This
    plugin used to include sidechain records so a subagent's spend counted
    against the ticket — but `isSidechain` is false on every record of every real
    transcript, so in practice it counted nothing and every subagent's tokens
    went unbilled to the ticket entirely. A subagent's full envelope arrives in the `Agent`
